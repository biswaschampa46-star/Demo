...
const insideCtg = /chittagong|chattogram|ctg/i.test(fields.city.trim());
const shipping = insideCtg ? FEE_INSIDE : FEE_OUTSIDE;
  ...
  /* â€”â€”â€” empty cart state (post-hydration) â€”â€”â€” */
        ...
        <div className="space-y-12 lg:col-span-7">
          ...
          <fieldset>
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <div>
                <label htmlFor="co-sender" className="label mb-2.5 block !tracking-[0.2em]">
                  Your {METHODS.find((m) => m.id === method)?.label} number <span className="normal-case tracking-normal text-mist/50">(for the delivery charge)</span>
                </label>
              ...
              <div>
                <label htmlFor="co-trx" className="label mb-2.5 block !tracking-[0.2em]">
                  Transaction ID <span className="normal-case tracking-normal text-mist/50">(for the delivery charge)</span>
                </label>
                <input id="co-trx" className="field" placeholder="e.g. 9HXK2â€¦" value={fields.transactionId} onChange={set("transactionId")} />
              </div>
            </div>
            ...
            <p className="mt-6 flex gap-3 rounded-2xl border border-line-soft bg-deep/40 p-5 text-xs leading-relaxed text-mist">
              ...
              Place your order first â€” payment instructions for your chosen method
          ...
          <div className="card-glass rounded-2xl p-8 lg:sticky lg:top-28">
            ...
            <dl className="mt-7 space-y-3.5 border-t border-line-soft pt-6 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-foam">{bdt(shipping)}</dd>
              </div>
            ...
            <button type="submit" disabled={state === "placing" || items.length === 0} className="btn btn-solid mt-7 w-full">
              {state === "placing" ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" /> Placing orderâ€¦
                </>
              ) : (
                <>
            ...
            <p className="mt-5 text-center text-[0.7rem] leading-relaxed text-mist/60">
              Advance payment only Â· no cash on delivery
            </p>
          </div>
        </aside>
      </form>
